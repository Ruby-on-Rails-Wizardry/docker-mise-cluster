...
# IMAGE default applies after dotenv. Priority (high → low):
#   shell export  >  .mise.env.local  >  .mise.env  >  ${FLAVOR_BASE}:dev
# Do not load config/compose.env here (generated by bin/compose; would stale-pin IMAGE).

# Preserve shell-provided IMAGE before files are applied.
_IMAGE_FROM_SHELL="__unset__"
if declare -p IMAGE &>/dev/null 2>&1; then
  _IMAGE_FROM_SHELL="${IMAGE}"
fi
...
# Always apply keys from file (override earlier file defaults).
...
load_dotenv_override "${ROOT}/.mise.env.local"
# Shell IMAGE=… wins over both files.
if [[ "${_IMAGE_FROM_SHELL}" != "__unset__" ]]; then
  export IMAGE="${_IMAGE_FROM_SHELL}"
fi
unset _IMAGE_FROM_SHELL
...
# Compose project name (Docker Desktop tree). Same for up / reheat / run.
# Priority: COMPOSE_PROJECT_NAME env > compose.yml top-level "name:" > directory basename.
compose_project_name() {
  ...
  local f name
  for f in "${ROOT}/compose.yml" "${ROOT}/compose.yaml"; do
    [[ -f "${f}" ]] || continue
    name="$(awk '
      /^[[:space:]]*#/ { next }
      /^name:[[:space:]]*/ {
